...
class PrecisionRecall(BaseMetric):
    ...
    def __str__(self):
        str_precision = [('Precision@%1.f_' % x) for x in self.k]
        str_recall = [('Recall@%1.f_' % x) for x in self.k]
        return (','.join(str_precision)) + ',' + (','.join(str_recall))

    def __call__(self, targets, predictions):
        precision, recall = zip(*[self._compute(targets, predictions, x) for x in self.k])
...
class MeanAP(BaseMetric):
    ...
    def __str__(self):
        return ','.join([('MeanAP@%1.f_' % x) for x in self.k])
...
class NormalizedDCG(BaseMetric):
    ...
    def __str__(self):
        return ','.join([('NDCG@%1.f_' % x) for x in self.k])
